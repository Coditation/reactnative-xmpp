  ...
  function onAcknowledgementRecived(data) {
    switch(self.unacked_command.command_type) {
      case COMMAND_TYPES.STREAM_SEND_MESSAGE:
        ...
        var dequeued_message = self.brocker_ack_queue.dequeue_element(command.data.newId, 'id');
        if(dequeued_message && dequeued_message.length > 0) {
...
Client.prototype.command_queue = new PersistentQueue.PersistentQueue(COMMAND_QUEUE_NAME)
...
Client.prototype.try_deque_command = function () {  
  ...
  this.unacked_command = this.command_queue.first()  
  if (this.unacked_command) {
...
Client.prototype.join = function (room, nick, cb) {
  ...
  try{
    ...
    var join_group_command = new JoinGroup(room, nick, stanza)
    this.queue_command(COMMAND_TYPES.JOIN_GROUP, join_group_command)
    return {
      message : "queued your request to join group"
    }
  }catch(e){
    return {
      "error":e
