...
class ReactNativeXmpp extends EventEmitter {
  constructor() {
    this.command_queue = new PersistentQueue(COMMAND_QUEUE_NAME);
    this.last_unacked_command;
  }
  ...
  - login: username
  - password: password
  - domain: XMPP domain to connect to 
  - host: Host to connect to
  - port: Port to connect to
  - resource: XMPP Resource used in the jid
  - brocker_ack_enabled: TBD
  - end_to_end_ack_enabled: TBD
  - is_stream_management_enabled: Enable stream management? true/false
  - is_soft_connection_monitoring_enabled: Enable soft connection state monitoring (using ping/pong)? true/false
  */
  ...
  send(send_message_command) {
    
  };

  join_group(join_group_command) {
    
  };

  _setup_event_handlers(client) {
    client.on('close', function(error) {
      // emit the event
      this.emit(ENUMS.EVENT_DISCONNECTED, error);
    });

    // TODO: handle the event associated with successful completion of the init sequence after
    // successfully establishing the socket
    client.on('a', function() {
      // emit the event
      this.emit(ENUMS.A);
    });
...
Client = function (conf) {
  ...
  function init() {
    ...
    ws = new WebSocket('wss://sunrise-stg-ejabberd.grupchat.site:5280/websocket');
